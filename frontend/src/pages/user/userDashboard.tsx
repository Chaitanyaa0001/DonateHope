...
const UserDashboard: React.FC = () => {
  ...
  if (error)
    return (
    <ErrorState title="Error Loading Monitors"message="We couldn’t load your monitors. Please refresh the page."/>
    );

  const totalAPIs = monitors.length;
  const avgUptime =monitors.length > 0? monitors.reduce((acc, cur) => acc + (cur.uptime ?? 0), 0) / monitors.length: 0;

  const avgLatency =monitors.length > 0? monitors.reduce((acc, cur) => acc + (cur.latency ?? 0), 0) / monitors.length: 0;
  const avgHealth =monitors.length > 0? monitors.reduce((acc, cur) => acc + (cur.score ?? 0), 0) / monitors.length: 0;
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants}>
      ...
      >
        <h1 className="text-4xl font-bold">Monitor <span className="text-purple-600">Dashboard</span></h1>
      ...
      >
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-gray-700 dark:text-gray-200 font-semibold">Total APIs</h2>
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-gray-700 dark:text-gray-200 font-semibold"> Avg Latency</h2>
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-gray-700 dark:text-gray-200 font-semibold">Health Score</h2>
          </div>
      ...
      <motion.div
        ...
        variants={itemVariants}
      >
        <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4">Response Time Trend</h2>
